...
export function useCyclePhase({ active, cycleMsBase, speed = 1 }: UseCyclePhaseOptions): number {
  ...
  useEffect(() => {
    if (!active) {
      // React's docs-recommended alternative (adjusting state during render
      // via a ref comparison) isn't available in this project — its React
      // Compiler config forbids ref reads/writes during render outright, a
      // stricter rule than plain React ships with. This effect genuinely is
      // synchronizing internal state with the `active` prop, so the
      // one-time reset on becoming inactive is the correct place for it.
...
export function useSteppedCycle({
  ...
  useEffect(() => {
    if (!active) {
      // See useCyclePhase's comment above — this project's React Compiler
      // config forbids the docs-recommended ref-during-render alternative,
      // so the reset stays here, in the effect that's genuinely
      // synchronizing this hook's state with the `active` prop.
